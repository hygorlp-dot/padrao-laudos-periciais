...
export type ReportSnapshot = {
  ...
  answers: Array<{ answer_id: string; section_id: string; question_id: string; text: string; finding_id: string; evidence_ids: string[]; method_ids: string[]; decision_id: string; claim_ids: string[]; question_text?: string }>;
...
export type ReportSourceCatalog = {
  sources: Array<{ kind: ReportSourceKind; id: string; label: string }>;
  context_sources: Record<"PROCESS_NUMBER" | "COURT" | "PARTIES" | "ADDRESSES" | "CLAIM_AND_GROUNDS" | "REQUESTS", Array<{ id: string; label: string }>>;
  questions: Array<{ question_id: string; text: string | null; findings: Array<{ finding_id: string; label: string }> }>;
};
