...
export function InspectionSessionView({ workspaceId }: { workspaceId: string }) {
  ...
  const submitItem = async (event: FormEvent) => {
    ...
    if (observation.trim()) { const id = recordId("OBS"); next.observations.push({ observation_id: id, inspection_item_id: item.item_id, observation_type: "DIRECT_OBSERVATION", raw_observation: observation.trim(), location_id: locationId, timestamp: now, operator: next.responsible_professional, provenance: `Registro direto da sessão ${next.session_id}.` }); item.observation_ids.push(id); }
    const hasMeasurement = [measurementQuantity, measurementValue, measurementUnit, instrumentIdentity, instrumentModel, instrumentSerial, methodName].some((value) => value.trim());
    if (hasMeasurement && [measurementQuantity, measurementValue, measurementUnit, instrumentIdentity, instrumentModel, instrumentSerial, methodName].every((value) => value.trim())) { const id = recordId("MEASUREMENT"); const instrumentId = recordId("INSTRUMENT"); const methodId = recordId("METHOD"); (next.instruments as object[]).push({ instrument_id: instrumentId, identity: instrumentIdentity.trim(), model: instrumentModel.trim(), serial_number: instrumentSerial.trim(), capability: measurementQuantity.trim(), calibration_claimed: false, certificate_reference: null }); (next.instrument_statuses as object[]).push({ status_id: recordId("INSTRUMENT-STATUS"), instrument_id: instrumentId, status: "NOT_CLAIMED", checked_at: now, evidence_reference: "Nenhuma alegação de calibração foi registrada." }); (next.methods as object[]).push({ method_id: methodId, name: methodName.trim(), procedure: "Procedimento identificado pelo profissional no registro de campo.", provenance: `Sessão ${next.session_id}.` }); next.measurements.push({ measurement_id: id, inspection_item_id: item.item_id, quantity: measurementQuantity.trim(), raw_value: measurementValue.trim(), raw_unit: measurementUnit.trim(), normalized_value: null, normalized_unit: null, instrument_id: instrumentId, method_id: methodId, location_id: locationId, timestamp: now, operator: next.responsible_professional, uncertainty: null, raw_observation: "Valor bruto informado pelo profissional.", provenance: `Registro bruto da sessão ${next.session_id}.` }); item.measurement_ids.push(id); }
