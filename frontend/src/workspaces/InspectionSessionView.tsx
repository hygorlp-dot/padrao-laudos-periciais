...
export function InspectionSessionView({ workspaceId }: { workspaceId: string }) {
    ...
    getInspectionSession(workspaceId, controller.signal).then(
      async (value) => {
        try {
          ...
          if (!controller.signal.aborted) { setState({ kind: "ready", value }); setSyncConflicts([{ code: "OFFLINE_REOPEN_FAILED", message: "O inventário offline não pôde ser reaberto.", record_ids: [], requires_explicit_review: true }]); }
        }
      },
  ...
  usePrefilledProfessional(expert, responsible, setResponsible);
  ...
  return <section className="inspection-workspace" aria-labelledby="inspection-title">
    ...
    <section className="planning-section"><h3>Itens executados</h3><ol className="inspection-items">{snapshot.items.map((item) => {
      ...
      return <li className="inspection-item" key={item.item_id}><header><div><strong>{item.title}</strong><span>{stateLabel[item.state]}</span></div><div><button className="text-action" type="button" onClick={() => { setSelectedItem(item.item_id); setItemState(item.state); setNote(item.note ?? ""); setSaveError(false); }}>Registrar campo</button></div></header>{item.note && <p>{item.note}</p>}
        {observations.length > 0 && <div><h4>Observações brutas</h4><ul>{observations.map((record) => <li key={record.observation_id}><strong>{sourceKindLabel(record.observation_type)}</strong> · {record.raw_observation}<small>{formatDateTime(record.timestamp)} · {record.provenance}</small><TechnicalDetails><span className="data">{record.observation_id} · local {record.location_id} · item planejado {item.planning_item_id}</span></TechnicalDetails></li>)}</ul></div>}
        ...
        {measurements.length > 0 && <div><h4>Medições</h4><ul>{measurements.map((record) => <li key={record.measurement_id}><strong>{record.raw_value} {record.raw_unit}</strong>{record.normalized_value && <span> · normalizado: {record.normalized_value} {record.normalized_unit}</span>}<small>{record.quantity} · {record.uncertainty ? `incerteza ${record.uncertainty}` : "incerteza não informada"} · {record.provenance}</small><TechnicalDetails><span className="data">{record.measurement_id} · instrumento {record.instrument_id} · método {record.method_id}</span></TechnicalDetails></li>)}</ul></div>}
        {photos.length > 0 && <div><h4>Fotografias</h4><ul>{photos.map((record) => <li key={record.photo_id}><strong>{record.caption}</strong><small>Original preservado · {record.device}</small><TechnicalDetails><span className="data">SHA-256 {record.original_sha256} · conteúdo {record.private_content_id} · {record.photo_id}</span></TechnicalDetails></li>)}</ul></div>}
        {limitations.length > 0 && <div className="inspection-limitations"><h4>Limitações</h4><ul>{limitations.map((record) => <li key={record.limitation_id}><strong>{sourceKindLabel(record.kind)}</strong><p>{record.description}</p><small>{record.consequence_for_coverage}</small></li>)}</ul></div>}
        {candidates.length > 0 && <div><h4>Evidências candidatas</h4><ul>{candidates.map((record) => <li key={record.candidate_id}><strong>Candidato à análise técnica — ainda não é achado</strong><p>{record.description}</p><small>{record.provenance}</small><TechnicalDetails><span className="data">{record.source_record_ids.join(" · ")}</span></TechnicalDetails></li>)}</ul></div>}
      </li>;
    })}</ol></section>
