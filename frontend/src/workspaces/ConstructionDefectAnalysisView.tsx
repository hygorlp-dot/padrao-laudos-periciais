...
import { formatDateTime, stateLabel } from "../ui/labels";
...
export function ConstructionDefectAnalysisView({ workspaceId }: { workspaceId: string }) {
  ...
  return <section className="pathology-workbench" aria-labelledby="pathology-title">
    ...
    {snapshot && <><ol className="pathology-list" aria-label="Patologias propostas">{snapshot.analysis_final.patologias.map((item) => { const latest = effectivePathologyReview(snapshot, item.id); return <li key={item.id}><article><header><h3>{item.id}</h3><span className={`pathology-status pathology-status--${latest?.action.toLowerCase() ?? "pending"}`}>{latest?.action === "APPROVE" ? "Aprovada pelo profissional" : latest?.action === "REJECT" ? "Rejeitada pelo profissional" : "Aguardando revisão"}</span></header><p>{item.manifestacao ?? "Manifestação sem descrição textual."}</p><p><strong>Conclusão técnica:</strong> {item.conclusao_tecnica ?? "[INFORMAÇÃO NECESSÁRIA: conclusão técnica]"}</p><p className="boundary-note">{item.evidencias?.length ? `${item.evidencias.length} ${item.evidencias.length === 1 ? "registro de campo vinculado" : "registros de campo vinculados"}` : "Nenhum registro de campo vinculado"}</p>{latest && <small>{latest.reason} · {formatDateTime(latest.reviewed_at)}</small>}{latest && <TechnicalDetails><span className="data">{latest.professional_id} · evidências {item.evidencias?.join(", ") || "—"}</span></TechnicalDetails>}</article></li>; })}</ol>
      <section className="technical-authority"><h3>Revisão profissional</h3><p>Cada revisão fica registrada sem apagar as anteriores. O padrão inicial é rejeitar; aprovar exige escolha expressa.</p><form onSubmit={review}><label>Patologia<select value={reviewPatId} onChange={(event) => setReviewPatId(event.target.value)}>{snapshot.analysis_final.patologias.map((item) => <option key={item.id} value={item.id}>{item.id} · {item.manifestacao ?? "sem descrição"}</option>)}</select></label><label>Ação profissional<select value={reviewAction} onChange={(event) => setReviewAction(event.target.value as "APPROVE" | "REJECT")}><option value="REJECT">Rejeitar</option><option value="APPROVE">Aprovar</option></select></label><label>Profissional responsável<input value={state.inspection.snapshot.responsible_professional} readOnly/></label><label>Fundamentação da revisão<textarea value={reviewReason} onChange={(event) => setReviewReason(event.target.value)}/></label><button className="authority-action" disabled={blocked}>Registrar revisão</button></form></section>
