...
export function PericialPlanningView({ workspaceId }: { workspaceId: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setState({ kind: "ready", value }); restoreReviewFocus.current = true; setSelected(null); setReviewer(expert?.profile_id ?? ""); setReason(""); setModified("");
    } catch { setSaveError(true); }
    finally { setSaving(false); }
  };
  return <section className="planning-workspace" aria-labelledby="planning-title">
    <header className="planning-overview"><div><h2 id="planning-title">Plano da perícia</h2><p>Propostas para preparar e executar a diligência. Nenhum item substitui a sua decisão profissional.</p></div><div className={`planning-readiness planning-readiness--${snapshot.coverage.readiness.toLowerCase()}`}><strong>{readinessLabel}</strong><span>{snapshot.coverage.reviewed_items} de {snapshot.coverage.material_items_total} itens revisados</span></div></header>
    ...
    <SiteLocationPanel workspaceId={workspaceId} />
