...
function PlanningCard({ item, method = false, onReview }: { item: PlanningItem; method?: boolean; onReview: (item: PlanningItem) => void }) {
  return <li className="planning-card"><div className="planning-card__heading"><div><strong>{item.title}</strong><span>{method && item.professional_review_status === "PENDING" ? "Método proposto — não aprovado" : statusLabel[item.professional_review_status]}</span></div><button className="text-action" type="button" onClick={() => onReview(item)}>Revisar {item.title}</button></div><p>{item.description}</p><details><summary>Por que este item existe?</summary><p>{item.derivation.rationale}</p><p>Itens da análise: {item.derivation.case_analysis_item_ids.join(", ")}</p><ul>{item.derivation.source_provenance.map((source) => <li key={source.occurrence_id}>{source.source_document_id} · {source.page_or_span} · {source.occurrence_id} · SHA {source.source_document_sha256}</li>)}</ul></details></li>;
}

function PlanningGroup({ title, items, methodItemIds, onReview }: { title: string; items: PlanningItem[]; methodItemIds?: ReadonlySet<string>; onReview: (item: PlanningItem) => void }) {
...
export function PericialPlanningView({ workspaceId }: { workspaceId: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setState({ kind: "ready", value }); setSelected(null); setReviewer(""); setReason(""); setModified("");
  ...
  return <section className="planning-workspace" aria-labelledby="planning-title">
    ...
    {groups.map(([title, items, methodIds]) => <PlanningGroup key={title} title={title} items={items} methodItemIds={methodIds} onReview={(item) => { setSelected(item); setAction("APPROVE"); setSaveError(false); }}/>) }
    ...
    {selected && <section className="planning-review" aria-labelledby="planning-review-title"><h3 id="planning-review-title">Revisar {selected.title}</h3><p>A proposta original será preservada no histórico.</p><form onSubmit={submit}><label>Decisão profissional<select value={action} onChange={(event) => setAction(event.target.value as ReviewAction)} disabled={saving || snapshot.upstream_stale}><option value="APPROVE">Aprovar</option><option value="REJECT">Rejeitar</option><option value="MODIFY">Modificar</option><option value="DEFER">Adiar</option></select></label><label>Identificação do perito<input value={reviewer} onChange={(event) => setReviewer(event.target.value)} required disabled={saving || snapshot.upstream_stale}/></label><label>Motivo da decisão<textarea value={reason} onChange={(event) => setReason(event.target.value)} required disabled={saving || snapshot.upstream_stale}/></label>{action === "MODIFY" && <label>Texto modificado<textarea value={modified} onChange={(event) => setModified(event.target.value)} required disabled={saving || snapshot.upstream_stale}/></label>}{saveError && <p role="alert">Não foi possível registrar a decisão. Reabra o plano e tente novamente.</p>}<div className="planning-review__actions"><button className="primary-action" type="submit" disabled={saving || snapshot.upstream_stale}>{saving ? "Registrando…" : "Registrar decisão"}</button><button className="text-action" type="button" onClick={() => setSelected(null)} disabled={saving}>Cancelar</button></div></form></section>}
  </section>;
}
