...
const snapshot = {
  ...
  template_id: "TEMPLATE-1", template_content_id: "22222222-2222-4222-8222-222222222222", template_format: "DOCM", template_revision: 1, template_digest: "b".repeat(64), rendering_version: "delivery-renderer/1",
...
describe("delivery foundation workbench", () => {
  test("shows explicit lifecycle, exact bindings, manifest hashes and local download without court filing", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(200, { revision: 6, updated_at: "2026-08-31T12:00:00Z", snapshot })));
    ...
    expect(screen.getByRole("link", { name: "Baixar laudo.pdf" })).toHaveAttribute("href", expect.stringContaining("/delivery-snapshot/artifacts/"));
  ...
  test("fails visibly stale and does not offer finalization", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(200, { revision: 6, updated_at: "2026-08-31T12:00:00Z", snapshot: stale })));
    render(<DeliveryFoundationView workspaceId={ID} />);
