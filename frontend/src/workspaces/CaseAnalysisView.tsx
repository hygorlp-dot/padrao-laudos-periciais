...
export function CaseAnalysisView({ workspaceId }: { workspaceId: string }) {
  ...
  usePrefilledProfessional(expert, reviewer, setReviewer);
  useEffect(() => {
  ...
  return <section className="analysis-workspace" aria-labelledby="analysis-map-title">
    ...
    <section className="analysis-section"><h3>Revisão humana</h3>{snapshot.human_reviews.length ? <p>{plural(snapshot.human_reviews.length, "decisão de revisão preservada", "decisões de revisão preservadas")} nesta análise.</p> : <p>Nenhuma revisão humana registrada.</p>}{allItems.length > 0 && <form onSubmit={submitReview}><label>Item<select value={reviewTarget} onChange={(event) => setReviewTarget(event.target.value)} required disabled={saving}><option value="">Selecione</option>{allItems.map((item) => <option key={item.item_id} value={item.item_id}>{item.text}</option>)}</select></label><label>Ação<select value={reviewAction} onChange={(event) => setReviewAction(event.target.value as typeof reviewAction)} disabled={saving}><option value="CONFIRM">Confirmar</option><option value="CORRECT">Corrigir</option><option value="REJECT">Rejeitar</option></select></label>{reviewAction === "CORRECT" && <label>Valor corrigido<textarea value={correction} onChange={(event) => setCorrection(event.target.value)} required disabled={saving}/></label>}<ProfessionalField label="Profissional revisor" value={reviewer} onChange={setReviewer} profile={expert} disabled={saving}/><label>Motivo<input value={reason} onChange={(event) => setReason(event.target.value)} required disabled={saving}/></label><button className="authority-action" type="submit" disabled={saving || snapshot.source_inventory_stale}>{saving ? "Registrando…" : "Registrar revisão"}</button></form>}</section>
  </section>;
}
