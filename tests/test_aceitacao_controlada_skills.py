...
class AceitacaoControladaSkillsTest(unittest.TestCase):
    ...
    def norma(self,limite=.5):return {"id":"NOR-001","titulo":"Norma sintética","requisito":"Abertura máxima sintética","verificada":True,"edicao":"2010","item":"1","pagina":1,"tipo_requisito":"DESEMPENHO","metodo_verificacao":"Medição direta","criterio":{"operador":"<=","valor":limite,"unidade":"mm","grandeza":"abertura"},"proveniencia":["https://oficial.example/norma"],"sistema":"IMPERMEABILIZACAO","classificacao_fonte":"FONTE_PRIMARIA_OFICIAL","entidade":"Entidade sintética","documento":"Norma sintética","dominio":"oficial.example","url":"https://oficial.example/norma","dominio_oficial":True,"status_verificacao":"VERIFICADO","escopo_fonte":"CONTEUDO_NORMATIVO","conteudo_normativo_verificado":True,"conteudo_consultado":"Abertura máxima sintética"}
    ...
    def test_e2e_norma_requisito_medicao_comparacao_claim_grounding(self):
        ...
        r=executar_pipeline_motor(self.processo,self.delim,self.plano,v,{"normas":[n]});pat=r["analise_final"]["patologias"][0];av=pat["normas_relacionadas"][0]["avaliacao_conformidade"]
        ...
        for valor,esperado in ((.8,"NAO_ATENDE"),(None,"INCONCLUSIVO")):
            ...
            obtido=executar_pipeline_motor(self.processo,self.delim,self.plano,variante,{"normas":[n]});self.assertEqual(obtido["analise_final"]["patologias"][0]["normas_relacionadas"][0]["avaliacao_conformidade"]["resultado"],esperado)
    ...
    def test_proposition_audit_verifica_requisito_nao_conformidade(self):
        claim={"id":"CLM-001","tipo":"REQUISITO_NORMATIVO"};g={"claim_id":"CLM-001","veredito":"GROUNDED","evidencias":["NOR-001"]};e={"id":"NOR-001","tipo":"NORMA","acessivel":True,"proveniencia":["https://oficial.example/norma"],"classe_probatoria":"EVIDENCIA_PRIMARIA","aspectos_suportados":["REQUISITO_NORMATIVO_VERIFICADO"],"aspectos_contraditos":[],"classificacao_fonte":"FONTE_PRIMARIA_OFICIAL","entidade":"Entidade sintética","documento":"Norma sintética","dominio":"oficial.example","url":"https://oficial.example/norma","dominio_oficial":True,"status_verificacao":"VERIFICADO","escopo_fonte":"CONTEUDO_NORMATIVO","conteudo_normativo_verificado":True,"conteudo_consultado":"requisito sintético","requisito":"requisito sintético"};r=auditar_proposicoes([claim],[g],[e])[0];self.assertEqual((r["verdict"],r["authority"]),("SUPPORTED","FONTE_PRIMARIA_OFICIAL"))
