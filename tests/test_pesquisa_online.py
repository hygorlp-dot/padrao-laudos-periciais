...
from scripts.conhecimento_privado.pesquisa_online import pesquisar,dominio_oficial,buscar_seguro,reconciliar_fontes,extrair_sucessao,cache_valido,AgentSearchProvider,MockSearchProvider
...
class PesquisaOnlineTest(unittest.TestCase):
    ...
    def test_busca_classifica_oficial_e_secundaria(self):
        ...
        r=buscar_seguro("ato",P());self.assertEqual(r["status"],"CONCLUIDA");self.assertTrue(r["resultados"][0]["oficial"]);self.assertEqual(r["resultados"][1]["status_uso"],"APENAS_DESCOBERTA")
    ...
    def test_timeout_sem_rede_e_erros_http(self):
        for erro,status in ((TimeoutError(),"TIMEOUT"),(OSError(),"SEARCH_PROVIDER_INDISPONIVEL"),(RuntimeError("HTTP 404"),"ERRO_PROVIDER"),(RuntimeError("HTTP 500"),"ERRO_PROVIDER")):
            ...
            self.assertEqual(buscar_seguro("consulta",P())["status"],status)
    ...
    def test_providers_mock_e_agente(self):
        mock=MockSearchProvider([{"url":"https://www.gov.br/fonte"}]);self.assertEqual(buscar_seguro("fonte pública",mock)["status"],"CONCLUIDA")
