...
def executar_pipeline_motor(processo,delimitacao,plano,vistoria,conhecimento=None,*,execucao_id=None):
    ...
    if not conhecimento.get("normas") and conhecimento.get("search_provider"):
        from scripts.conhecimento_privado.pesquisa_online import buscar_seguro
        pesquisa=buscar_seguro("fonte oficial pública aplicável à questão técnica",conhecimento["search_provider"])
    ...
    final,correcoes=autocorrigir(inicial,claims,audits,detector+deep);final["catalogo_evidencias"]=catalogo;final["autoauditoria"]=auditar(final,vistoria)
    ...
    bloqueantes=bool(erros_finais or detector_f or prop_bloqueante or medicao_indispensavel_ausente or any(a["veredito"] in REPROVADOS|{"UNVERIFIABLE"} for a in materiais) or any(a.get("bloqueante") for a in final["autoauditoria"]))
    ressalvas=any(p.get("ressalvas") or p.get("analise_causal",{}).get("grau_certeza")=="INCONCLUSIVO" for p in final.get("patologias",[]))
    gate="BLOQUEADO_PARA_REDACAO" if bloqueantes else "APTO_PARA_REDACAO_COM_RESSALVAS" if ressalvas else "APTO_PARA_REDACAO";final["gate_redacao"]=gate
