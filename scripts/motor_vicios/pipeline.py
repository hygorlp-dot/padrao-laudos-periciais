...
def recalcular_gate(estado,status_declarado=None):
    ...
    bloqueio=bool(estado.get("erros") or estado.get("detector") or estado.get("proposition_bloqueante") or estado.get("medicao_ausente") or any(a.get("veredito") in REPROVADOS|{"UNVERIFIABLE"} for a in materiais) or any(a.get("bloqueante") for a in estado.get("autoauditoria",[])))
    return "BLOQUEADO_PARA_REDACAO" if bloqueio else "APTO_PARA_REDACAO_COM_RESSALVAS" if estado.get("ressalvas") else "APTO_PARA_REDACAO"
...
def executar_pipeline_motor(processo,delimitacao,plano,vistoria,conhecimento=None,*,execucao_id=None):
    ...
    if not conhecimento.get("normas") and conhecimento.get("search_provider"):
        ...
        pesquisa=buscar_seguro("fonte oficial pública aplicável à questão técnica",conhecimento["search_provider"],EgressPolicy(permitir_egress=True))
    ...
    obrigatorias={m["id"] for m in plano.get("medicoes",[]) if m.get("obrigatoriedade")=="OBRIGATORIA"}
    medicao_indispensavel_ausente=any(c.get("tipo")=="MEDICAO" and c.get("planejado") in obrigatorias and c.get("status")=="NAO_EXECUTADO" and not c.get("evidencia_equivalente") for c in vistoria.get("cobertura",[]))
    ressalvas=any(p.get("ressalvas") or p.get("analise_causal",{}).get("grau_certeza")=="INCONCLUSIVO" for p in final.get("patologias",[]))
    gate=recalcular_gate({"erros":erros_finais,"detector":detector_f,"proposition_bloqueante":prop_bloqueante,"medicao_ausente":medicao_indispensavel_ausente,"materiais":materiais,"autoauditoria":final["autoauditoria"],"ressalvas":ressalvas},status_declarado=final.get("gate_redacao"));final["gate_redacao"]=gate
    ...
    trilha=registrar_trilha(execucao_id=execucao_id or str(uuid.uuid4()),processo=processo.get("numero_processo"),skill="motor-vicios-construtivos",objetivo="Gerar, auditar, corrigir e liberar análise técnica",inputs=["processo","delimitacao","plano","vistoria","conhecimento"],fontes=sorted({x for e in catalogo_f for x in e.get("proveniencia",[])}),artefatos=["PAT_INICIAL","PAT_FINAL"],decisoes_autonomas=[{"decisao":"Gate pós-auditoria","evidencia":str(len(audits_f)),"justificativa":"Somente o estado final validado controla a liberação"}],outputs=["analise_final"],auditorias=[a["claim_id"]+":"+a["veredito"] for a in audits_f]+[p["claim_id"]+":PROPOSITION:"+p["verdict"] for p in proposition],achados=[{"tipo":a.get("tipo"),"severidade":a.get("severidade")} for a in detector_f+deep_f],correcoes=correcoes,status="BLOQUEADO" if gate=="BLOQUEADO_PARA_REDACAO" else "APROVADO",proveniencia=final.get("proveniencia",[]))
    return {"analise_inicial":inicial,"erros_iniciais":erros_iniciais,"claims":claims,"grounding_inicial":audits,"detector_inicial":detector,"deep_audit_inicial":deep,"autocorrecoes":correcoes,"analise_final":final,"claims_finais":claims_f,"grounding_final":audits_f,"detector_final":detector_f,"deep_audit_final":deep_f,"erros_finais":erros_finais,"proposition_audit":{"modo":"WRAPPER_LOCAL","resultados":proposition},"claim_audit":{"modo":"INTEGRACAO_METODOLOGICA_ADAPTADA","resultados":audits_f},"proposition_audit_results":proposition,"autoauditoria_resultado":resultado_autoauditoria,"trilha":trilha,"gate":gate}
