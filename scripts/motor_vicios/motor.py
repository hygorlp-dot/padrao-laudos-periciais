...
from .normas import recuperar_normas_para_manifestacao,avaliar_conformidade_normativa,normalizar_fonte_normativa,projetar_norma_pat
...
def _agrupar_por_manifestacao(observacoes):
    por_manifestacao={}
    for o in observacoes:
        chave=(o.get("manifestacao") or "Condição examinada",o.get("ambiente"),o.get("sistema"),o.get("elemento"))
        por_manifestacao.setdefault(chave,[]).append(o)
    return por_manifestacao


def _chave_ordenacao_manifestacao(chave):
    return tuple(str(v or "").casefold() for v in chave)


def _construir_contextos_associacao(por_manifestacao):
    contextos=[]
...
def _construir_manifestacao_e_patologia(chave,por_manifestacao,catalogo,r,contexto,conhecimento,hip_seq):
    ...
    manifestacao={"id":mid,"descricao":desc,"sistema":sis,"elemento":elem,"local":amb,"alegacoes":alg,"observacoes":oids,"fotografias":fot,"medicoes":med,"questoes":qts}
    ...
    return manifestacao,pat,hips


def _recalcular_patologia(p,r,catalogo,conhecimento):
...
def _sanear_questao(qt,r):
    ...
    entrada["dimensoes_saneamento"]=[{"intencao":d,"status":"NAO_SANEADA_POR_CAPACIDADE" if d in {"CAUSALIDADE","ORIGEM","MECANISMO"} and sem_capacidade else "SANEADA" if pats else "PREJUDICADA"} for d in (dimensoes or ["CARACTERIZACAO"])]
    return entrada


def _agregar_cobertura(c,porqt):
...
def _calcular_autonomia(r):
    return {"decisoes_autonomas":len(r["patologias"])+len(r["questoes_saneadas"]),"hipoteses_criadas":len(r["hipoteses"]),"hipoteses_testadas":len(r["hipoteses"]),"hipoteses_fortalecidas":sum(h["status"] in {"FORTALECIDA","MAIS_PROVAVEL"} for h in r["hipoteses"]),"hipoteses_afastadas":sum(h["status"] in {"ENFRAQUECIDA","AFASTADA"} for h in r["hipoteses"]),"ressalvas_criadas":sum(bool(p["ressalvas"]) for p in r["patologias"]),"normas_recuperadas":sum(len(p["normas_relacionadas"]) for p in r["patologias"])}
...
def executar(processo,delimitacao,plano,vistoria,contexto=None,conhecimento=None):
    ...
    if vistoria.get("status")=="AGUARDANDO_DADOS_DE_VISTORIA" or not vistoria.get("observacoes"):
        ...
        return bloqueado
    r=_base(processo,delimitacao,"ANALISE_INICIAL",tipo); contexto=contexto or {};conhecimento=conhecimento or {};catalogo=construir_catalogo(vistoria,conhecimento.get("documentos"),conhecimento.get("normas"));r["catalogo_evidencias"]=catalogo;r["estado_analise"]="PAT_INICIAL"
    por_manifestacao=_agrupar_por_manifestacao(vistoria["observacoes"])
    r["relacoes_associacao"]=associar_documentos_ensaios(catalogo,_construir_contextos_associacao(por_manifestacao))
    hip_seq=1
    for chave in sorted(por_manifestacao,key=_chave_ordenacao_manifestacao):
        manifestacao,pat,hips=_construir_manifestacao_e_patologia(chave,por_manifestacao,catalogo,r,contexto,conhecimento,hip_seq)
        hip_seq+=len(hips)
        r["manifestacoes"].append(manifestacao);r["hipoteses"].extend(hips);r["patologias"].append(pat)
    for p in r["patologias"]:
        _recalcular_patologia(p,r,catalogo,conhecimento)
    for qt in delimitacao["questoes_tecnicas"]:
        r["questoes_saneadas"].append(_sanear_questao(qt,r))
    porqt={q["id"]:q for q in r["questoes_saneadas"]}
    for c in r["cobertura_quesitos"]:
        _agregar_cobertura(c,porqt)
    r["autoauditoria"]=auditar(r,vistoria);r["gate_redacao"]="BLOQUEADO_PARA_REDACAO";r["autonomia"].update(_calcular_autonomia(r));return r
