...
if ([int]$identity.port -ne $Port) {
    Write-Error "Requested port does not match managed bridge metadata; no process was stopped."
    exit 1
}
...
try {
    ...
    $endpointAlive = $health.status -eq 'ok'
...
if ($processInfo -and $healthMatches -and $identity.module -eq 'scripts.agentic.claw3d.bridge' -and
    ...
    exit 1
} elseif ($endpointAlive) {
    Write-Error 'PID metadata is stale while a bridge endpoint remains active; no process was stopped.'
    exit 1
}
