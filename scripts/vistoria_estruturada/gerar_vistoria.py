...
def _processar_arquivo(a,planejadas,plano,fotos,videos,docs):
...
def _processar_linha(r,a,plano,medicoes,atividades,ensaios,declaracoes,limitacoes,observacoes):
    ...
    if tipo in {"ATV","ATIVIDADE"} and descricao:
        ...
        atividades.append({"id":f"ATV-EXEC-{len(atividades)+1:03d}","atividade_planejada":r.get("atividade_planejada"),"descricao":descricao,"status":r.get("status","EXECUTADO"),"questoes":planejada.get("questoes_tecnicas",[]),"evidencias":[a["id"]],"impacto_nao_execucao":None});return
    if tipo in {"ENS","ENSAIO"} and descricao:
        ...
        ensaios.append({"id":f"ENS-{len(ensaios)+1:03d}","ensaio_planejado":planejado if item else None,"nome":descricao,"status":r.get("status","EXECUTADO"),"resultado":r.get("resultado"),"unidade":r.get("unidade"),"metodo":r.get("metodo"),"questoes":item.get("questoes_tecnicas",[]),"proveniencia":[a["id"]]});return
...
def _comp(item,obs,registros,atividades_univocas):
...
def _registrar_relacoes(obs,medicoes,fotos,registros,atividades_univocas,relacoes):
    obs["medicoes"]=sorted(m["id"] for m in medicoes if _comp(m,obs,registros,atividades_univocas));obs["fotografias"]=sorted(f["id"] for f in fotos if _comp(f,obs,registros,atividades_univocas))
...
def _montar_executados(fotos,medicoes,atividades,ensaios,docs,plano):
...
def _montar_cobertura(plano,executados):
    cobertura=[]
    if plano:
...
def gerar(inventario,plano=None,numero_processo=None):
    fotos=[];videos=[];docs=[];medicoes=[];observacoes=[];declaracoes=[];limitacoes=[];atividades=[];ensaios=[];planejadas=(plano or {}).get("fotografias",[])
    for a in sorted(inventario["arquivos"],key=lambda x:x.get("id","")):
        _processar_arquivo(a,planejadas,plano,fotos,videos,docs)
        for r in _linhas(a):
            _processar_linha(r,a,plano,medicoes,atividades,ensaios,declaracoes,limitacoes,observacoes)
    relacoes=[]
    registros=_univocos(observacoes,"_registro_id")
    atividades_univocas=_univocos(observacoes,"atividade")
    for obs in observacoes:
        _registrar_relacoes(obs,medicoes,fotos,registros,atividades_univocas,relacoes)
    for obs in observacoes:obs.pop("_registro_id",None)
    for med in medicoes:med.pop("_vinculo_registro",None)
    executados=_montar_executados(fotos,medicoes,atividades,ensaios,docs,plano)
    cobertura=_montar_cobertura(plano,executados)
